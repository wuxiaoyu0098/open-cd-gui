...
class PyramidFeatureFusion(nn.Module):
    ...
    def forward(self, feas, ds_feas):
        ...
        # 特征图尺寸已经在 DenseAdapterLite 中统一，不需要再次对齐
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # 获取 FPN 特征的空间尺寸作为目标尺寸
        target_sizes = [f.shape[-2:] for f in fea]  # [(H1, W1), (H2, W2), (H3, W3), (H4, W4)]
        
        # process dense features，使用 FPN 特征的实际尺寸
        ds_fea = self.dense_adp(ds_fea, target_sizes=target_sizes)
...
class Detector(nn.Module):
    ...
    def forward(self, x1s, x2s, target_size=None):
        ...
        # 使用目标尺寸（输入图像尺寸），如果没有提供则使用 p2 的尺寸
        if target_size is None:
            target_size = pred_p2.shape[-2:]
        target_h, target_w = target_size

        pred_p2 = F.interpolate(
            pred_p2, size=(target_h, target_w), mode="bilinear", align_corners=False
        )
        pred_p3 = F.interpolate(
            pred_p3, size=(target_h, target_w), mode="bilinear", align_corners=False
        )
        pred_p4 = F.interpolate(
            pred_p4, size=(target_h, target_w), mode="bilinear", align_corners=False
        )
        pred_p5 = F.interpolate(
            pred_p5, size=(target_h, target_w), mode="bilinear", align_corners=False
        )
...
class ChangeModel(nn.Module):
    ...
    def _forward(self, x1, x2):
        ...
        pred, _, _, _ = self.detector(fea1, fea2, target_size=target_size)
    ...
    def forward(self, x1, x2):
        ...
        preds = self.detector(fea1, fea2, target_size=target_size)

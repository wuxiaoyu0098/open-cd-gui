...
def BCEDiceLoss(inputs, targets):
    ...
    Returns:
        Combined loss value
    """
    # Under AMP (precision=16-mixed), BCE on probabilities is unsafe to autocast.
    # Compute BCE/Dice in float32 with autocast disabled for numerical stability.
    ...
    with autocast_ctx:
        # CUDA BCE requires finite probabilities in (0, 1) for stable kernels; NaN/Inf
        # from AMP or diverged weights must be cleaned (clamp alone does not fix NaN).
        bce_eps = 1e-7
        inputs_f = torch.nan_to_num(inputs.float(), nan=0.5, posinf=1.0, neginf=0.0)
        inputs_f = inputs_f.clamp(bce_eps, 1.0 - bce_eps)
        targets_f = torch.nan_to_num(targets.float(), nan=0.0, posinf=1.0, neginf=0.0)
        targets_f = targets_f.clamp(0.0, 1.0)
        bce = F.binary_cross_entropy(inputs_f, targets_f)
        inter = (inputs_f * targets_f).sum()
        dice_eps = 1e-5
        dice = (2 * inter + dice_eps) / (inputs_f.sum() + targets_f.sum() + dice_eps)
        return bce + 1 - dice
...
def setup_logger(args, save_path):
    ...
    elif 'MCD' in args.dataset:
        logger.write("\n%s\t%s\t%s\t%s" %
            ('epoch', 'loss_val', 'mIoU_val', 'pixAcc_val'))

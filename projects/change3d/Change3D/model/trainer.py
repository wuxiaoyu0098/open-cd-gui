...
class Trainer(nn.Module):
    ...
    def __init__(self, args: Any) -> None:
        ...
        # For multi-class change detection (change type segmentation)
        if args.num_perception_frame == 1 and 'MCD' in args.dataset:
            # Outputs logits with args.num_class channels (no sigmoid)
            self.decoder = ChangeDecoder(args, in_dim=self.embed_dims, has_sigmoid=False)
            weight_init(self.decoder)

        # For binary change detection and change caption task
        elif args.num_perception_frame == 1 and ('CD' in args.dataset and 'MCD' not in args.dataset):
    ...
    def update_mcd(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Multi-class change type prediction.

        Returns:
            Logits tensor with shape [B, num_class, H, W]
        """
        features = self.encoder(x, y)
        perception_change_feat = list(map(lambda x: x[0], features))
        logits = self.decoder(perception_change_feat)
        return logits
    ...
    def update_bda(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through the complete model.
        把输入前后时相图像编码成两套 perception 特征，再分别喂给两个解码头，输出“分类图 + 二值定位图”。

...
# Keep this before importing Change3D/pytorchvideo. In this Windows env,
# importing torchvision after torch can make Pillow's _imaging DLL fail.
os.environ.setdefault("PYTHONNOUSERSITE", "1")
from PIL import Image  # noqa: F401,E402
import torchvision.transforms.functional as TF  # noqa: F401,E402
...
class Change3DBCDExport(nn.Module):
    ...
    def forward(self, pre_img: torch.Tensor, post_img: torch.Tensor) -> torch.Tensor:
        ...
        return self.trainer.update_bcd(pre_img, post_img)
...
def _compare_silu_swish(
    ...
    mod_silu = Change3DBCDExport(trainer_silu).to(device).eval()
    mod_swish = Change3DBCDExport(trainer_swish).to(device).eval()
...
def export_bcd_ckpt_to_pt(
    weights: str,
    out_pt: str,
    dataset: str = "Change3D-CD",
    in_height: int = 512,
    in_width: int = 512,
    pretrained: str = "",
    cuda_no: int = -1,
) -> str:
    if cuda_no >= 0 and torch.cuda.is_available():
        os.environ["CUDA_VISIBLE_DEVICES"] = str(cuda_no)
        device = torch.device("cuda:0")
    else:
        device = torch.device("cpu")

    pretrained = pretrained or os.path.join(_ROOT, "model", "X3D_L.pyth")
    model_args = build_bcd_args(dataset, in_height, in_width, pretrained,
                                num_class=1)
    trainer = Trainer(model_args).to(device).float()

    try:
        raw = _torch_load(weights, map_location=device, weights_only=True)
    except Exception:
        raw = _torch_load(weights, map_location=device, weights_only=False)
    if isinstance(raw, dict) and "state_dict" in raw:
        raw = raw["state_dict"]
    trainer.load_state_dict(_strip_module_prefix(raw), strict=True)
    trainer.eval()

    export_mod = Change3DBCDExport(trainer).to(device).eval()
    _export_jit(export_mod, device, in_height, in_width, out_pt)
    return out_pt
...
def main() -> None:
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default=r"D:\Projects\change_detection\open-cd-main\results\change3d\bcd\Change3D_BCD_TrainingFiles\best-model-step=000074-val_iou=0.2388.ckpt",
        help="仅含 state_dict 的 .pth（如 best_model*.pth）",
    )
    parser.add_argument(
        "--out_pt",
        type=str,
        default=r"D:\Projects\change_detection\open-cd-main\results\change3d\bcd\Change3D_BCD_TrainingFiles\best-model-step=000074-val_iou=0.2388.pt",
    ...
    parser.add_argument("--num_class", type=int, default=1, help="MCD 输出类别数（与训练一致）")
    parser.add_argument(
        "--pretrained",
        type=str,
